...
class ProductIntent(BaseModel):
    ...
    - hard_constraints: Must satisfy (price ranges, ratings, specific features)
    ...
    product: str = Field(..., description="Main product name")
    ...
    hard_constraints: Dict[str, Any] = Field(
        default_factory=dict,
        description="Must-have requirements like price range, min rating, specific features"
    )
